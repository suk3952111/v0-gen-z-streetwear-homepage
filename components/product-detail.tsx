...
import { ImageFocusModal, type FocusImage } from "@/components/product/image-focus-modal"
...
// Generate gallery images from the product's main image with variations
...
export function ProductDetail({ language, productId }: ProductDetailProps) {
  ...
  // Build product focus images
  const productFocusImages: FocusImage[] = useMemo(() => {
    if (!product) return []
    return galleryImages.map((src, i) => ({
      id: `product-${productId}-${i}`,
      src,
      alt: `${product.name} - ${language === "KR" ? "이미지" : "Image"} ${i + 1}`,
      type: "product" as const,
    }))
  }, [product, galleryImages, productId, language])
  ...
  // Open focus mode for product images
  ...
  // Open focus mode from review images (called by ReviewSection)
  const openReviewFocus = useCallback((reviewImages: FocusImage[], startIndex: number) => {
    // Combine product images + review images for full slider
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] pt-20">
        ...
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-0">
          ...
          <div className="w-full lg:w-1/2 lg:pr-8">
            ...
            <motion.div
              ...
              aria-label={`${language === "KR" ? "클릭하여 이미지 확대" : "Click to zoom image"}`}
            >
              ...
              />
              <Image
                src={galleryImages[activeThumb] || "/placeholder.svg"}
                alt={product.name}
                fill
              ...
              <div className="absolute inset-0 z-20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                <div className="px-4 py-2 bg-[#0a0a0a]/80 border-2 border-[#CCFF00] text-[#CCFF00] text-sm font-bold uppercase tracking-wider">
                  {language === "KR" ? "클릭하여 확대" : "CLICK TO FOCUS"}
                </div>
              </div>
              {/* Tags on image */}
            ...
            <div className="flex gap-3 mt-4">
              {galleryImages.map((img, index) => (
                <motion.button
                  ...
                  layoutId={
                    index !== activeThumb
                      ? `focus-image-product-${productId}-${index}`
                      : undefined
                  }
                  onClick={() => setActiveThumb(index)}
                  className={`relative w-1/4 aspect-square border-4 overflow-hidden transition-all ${
                    activeThumb === index
                      ? "border-[#CCFF00] shadow-[0_0_15px_rgba(204,255,0,0.3)]"
                      : "border-[#333333] hover:border-[#CCFF00]/50 opacity-60 hover:opacity-100"
                  }`}
          ...
          <div className="w-full lg:w-1/2 lg:pl-8 lg:border-l-4 lg:border-[#CCFF00]">
            ...
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold text-white uppercase tracking-tighter mb-4 text-balance">
              {product.name}
            </h1>
              ...
              >
                ...
                <div className="p-4 border-t-4 border-[#CCFF00]">
                  ...
                  <p className="text-[#CCFF00] text-xl font-bold">
                    {formatPrice(language === "KR" ? sp.price : sp.priceUSD, language === "KR" ? "KRW" : "USD")}
                  </p>
                </div>
              </Link>
            ))}
      ...
      <ReviewSection
        productId={productId}
        language={language}
        onImageClick={openReviewFocus}
      />

      {/* Image Focus Modal */}
      <ImageFocusModal
        images={allFocusImages}
