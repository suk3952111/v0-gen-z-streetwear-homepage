...
const analyzeImageStyle = async (imageDataUrl: string): Promise<ImageStyleAnalysis> => {
  const image = parseDataUrl(imageDataUrl)
  const content = await callGeminiGenerate([
    {
      text:
        "Analyze this fashion image and return strict JSON only: {\"caption\":\"short caption\",\"style_tags\":[\"tag1\",\"tag2\",\"tag3\"]}. style_tags must be short lowercase words, max 8.",
    },
    {
      inline_data: {
  ...
  const parsed = parseJsonSafe<{ caption?: string; style_tags?: string[] }>(content, {})
...
const ensureProductImageCaptionEmbeddings = async (
  ...
  for (const row of rows) {
    try {
      ...
      const { error: updateError } = await updateProductImageCaptionEmbeddingQueryBuilder(
        supabaseClient,
        row.image_id,
        analysis.caption,
        embeddingLiteral,
...
const mapMatchedProductsToItems = async (
  ...
  const { data: productsData, error: productsError } = await getProductsByIdsQueryBuilder(
    supabaseClient,
    productIds,
  )
  if (productsError) {
...
export const recommendSimilarProductsByAi = async (
  ...
  const sourceProductDbId = await getProductDbIdBySlug(supabaseClient, input.productId)
